class RemoteVisionApp {
    constructor() {
        this.config = {
            peerServer: {
                ...
                config: {'iceServers': [{urls:'stun:stun.l.google.com:19302'},{urls:'stun:global.stun.twilio.com:3478'},{urls:'turn:numb.viagenie.ca',credential:'muazkh',username:'webrtc@live.com'}]}
            },
            videoQuality: {
        ...
        this.state = {
            isEmitter: false, isViewer: false, isConnected: false, isStreaming: false,
            isAudioEnabled: false, isFrontCamera: true, isFlipped: false, currentCode: null,
            displayCode: null, peerId: null, hostPeerId: null, connectedViewers: new Set(),
            recentCodes: JSON.parse(localStorage.getItem('recentCodes')) || [],
            settings: JSON.parse(localStorage.getItem('visionSettings')) || {videoQuality:'medium',autoReconnect:true},
            connectionAttempts: 0
        };
        this.peer = null; this.localStream = null; this.remoteStream = null;
        this.currentCall = null; this.dataConnection = null; this.mediaDevices = [];
        this.currentDeviceId = null; this.connectionTimer = null; this.uptimeInterval = null;
        this.connectionStartTime = null; this.statsInterval = null; this.localAudioStream = null;
        this.connectionTimeouts = []; this.supervisorAudioElement = null;
        this.elements = {
    ...
    async initializePeerJS(customId = null) {
        return new Promise((resolve, reject) => {
            ...
            try {
                this.peer = customId ? new Peer(customId, this.config.peerServer) : new Peer(this.config.peerServer);
                ...
                this.peer.on('error', (err) => {
                    ...
                    switch(err.type) {
                        case 'peer-unavailable': errorMsg='El código ingresado no existe o ha expirado'; break;
                        case 'network': errorMsg='Error de red. Verifica tu conexión a internet'; break;
                        case 'server-error': errorMsg='Error del servidor. Intenta recargar la página'; break;
                        case 'disconnected': errorMsg='Desconectado del servidor'; break;
                    }
                    if (err.type !== 'disconnected') { reject(errorMsg); }
                });
                ...
                const initTimeout = setTimeout(() => {
                    if (!this.state.peerId) { reject('Timeout al conectar con el servidor PeerJS'); }
                }, 15000);
    ...
    setupEventListeners() {
        ...
        this.elements.peerCodeInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') this.connectToEmitter(); });
        ...
        this.elements.errorModal.addEventListener('click', (e) => { if (e.target === this.elements.errorModal) this.hideErrorModal(); });
    }
    generateDisplayCode() {
        const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; let code = '';
        for (let i = 0; i < 6; i++) { code += chars.charAt(Math.floor(Math.random() * chars.length)); }
        this.state.displayCode = code; this.elements.emitterCode.textContent = code;
    ...
    loadRecentCodes() {
        const container = this.elements.recentCodes; container.innerHTML = '';
        if (this.state.recentCodes.length === 0) return;
        this.state.recentCodes.forEach(code => {
            const button = document.createElement('button');
            button.className = 'recent-code'; button.textContent = code;
    ...
    async setEmitterMode() {
        try {
            ...
            this.state.isEmitter = true; this.state.isViewer = false;
    ...
    async setViewerMode() {
        try {
            ...
            this.state.isEmitter = false; this.state.isViewer = true;
            this.elements.modeSelection.classList.add('hidden');
    ...
    showModeSelection() {
        if (this.state.isEmitter) { this.stopEmitter(); } else { this.disconnectFromEmitter(); }
        if (this.peer) { this.peer.destroy(); this.peer = null; }
        this.clearAllTimeouts();
        this.state.isEmitter = false; this.state.isViewer = false;
        this.state.isConnected = false; this.state.peerId = null;
        this.elements.emitterPanel.classList.add('hidden');
        this.elements.viewerPanel.classList.add('hidden');
    ...
    async startEmitter() {
        try {
            ...
            const constraints = {
                video: { ...this.config.videoQuality[this.state.settings.videoQuality], facingMode: facingMode, deviceId: this.currentDeviceId ? { exact: this.currentDeviceId } : undefined },
    ...
    stopEmitter() {
        if (this.localStream) { this.localStream.getTracks().forEach(track => track.stop()); this.localStream = null; this.elements.localVideo.srcObject = null; }
        ...
        if (this.supervisorAudioElement) { this.supervisorAudioElement.remove(); this.supervisorAudioElement = null; }
        this.elements.btnStartEmitter.classList.remove('hidden'); this.elements.btnStopEmitter.classList.add('hidden'); this.state.isStreaming = false;
        this.state.connectedViewers.clear(); this.updateConnectedClients();
    ...
    async switchCamera() {
        ...
        try {
            await this.getMediaDevices();
            if (this.mediaDevices.length < 2) { this.showNotification('No se encontró otra cámara', 'warning'); return; }
            this.state.isFrontCamera = !this.state.isFrontCamera;
            const currentTrack = this.localStream.getVideoTracks()[0]; currentTrack.stop();
            const facingMode = this.state.isFrontCamera ? 'user' : 'environment';
            const device = this.mediaDevices.find(d => d.label.toLowerCase().includes(facingMode === 'user' ? 'front' : 'back') || d.label.toLowerCase().includes(facingMode === 'user' ? 'delantera' : 'trasera')) || this.mediaDevices[0];
            this.currentDeviceId = device.deviceId;
            const newStream = await navigator.mediaDevices.getUserMedia({
                video: { ...this.config.videoQuality[this.state.settings.videoQuality], deviceId: { exact: device.deviceId } },
                audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 2 }
            });
            const newVideoTrack = newStream.getVideoTracks()[0];
            const audioTrack = this.localStream.getAudioTracks()[0];
            this.localStream.removeTrack(currentTrack); this.localStream.addTrack(newVideoTrack);
            if (this.currentCall) {
            ...
            console.error('Error al cambiar cámara:', error); this.showNotification('Error al cambiar cámara', 'error');
        }
    }
    flipCamera() {
    ...
    async getMediaDevices() {
        try {
            ...
            if (this.mediaDevices.length > 0 && !this.currentDeviceId) {
                const frontCamera = this.mediaDevices.find(device => device.label.toLowerCase().includes('front') || device.label.toLowerCase().includes('user') || device.label.toLowerCase().includes('delantera'));
    ...
    toggleEmitterAudio() {
        ...
        if (audioTrack) {
            ...
            this.elements.btnAudioToggle.innerHTML = `<i class="fas fa-microphone${isEnabled ? '' : '-slash'}"></i> Micrófono: ${isEnabled ? 'ON' : 'OFF'}`;
    ...
    async connectToEmitter() {
        ...
        this.state.hostPeerId = code; this.state.connectionAttempts = 0;
    ...
    async attemptConnection(code) {
        try {
            this.clearAllTimeouts();
            if (!this.peer || this.peer.disconnected) { throw new Error('No conectado al servidor PeerJS'); }
    ...
    async attemptDataConnection(code) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => { reject(new Error('Timeout de conexión de datos (10 segundos)')); }, 10000);
            this.connectionTimeouts.push(timeout);
            const dataConn = this.peer.connect(code, { reliable: true, serialization: 'json' });
            dataConn.on('open', () => {
                clearTimeout(timeout);
                console.log('✅ Conexión de datos abierta con:', dataConn.peer);
                this.dataConnection = dataConn;
                dataConn.on('data', (data) => { this.handleDataMessage(data); });
                dataConn.on('close', () => {
                    console.log('Conexión de datos cerrada');
                    this.dataConnection = null;
                    if (this.state.isConnected) { this.handleDisconnection(); }
                });
                dataConn.on('error', (err) => { console.error('Error en conexión de datos:', err); });
                resolve();
            });
    ...
    async attemptVideoCall(code) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // 🔥 CORRECCIÓN CRÍTICA: Obtener un stream local ANTES de llamar.
    ...
    async createLocalAudioStream() {
        ...
        try {
            ...
            this.localAudioStream.getAudioTracks().forEach(track => { track.enabled = false; }); // Silenciado por defecto
    ...
    handleRemoteStream(stream) {
        ...
        if (videoTracks.length === 0) { this.showNotification('Advertencia: No se detectó video en la transmisión', 'warning'); }
        if (audioTracks.length === 0) { this.showNotification('Advertencia: No se detectó audio en la transmisión', 'warning'); } else { audioTracks[0].enabled = true; }
        ...
        this.startUptimeTimer(); this.startConnectionStatsMonitoring();
        this.showNotification('¡Conectado a la transmisión!', 'success');
        this.updateConnectionStats();
    }
    setupVideoForMobile() {
        const video = this.elements.remoteVideo;
        video.setAttribute('playsinline', 'true'); video.setAttribute('webkit-playsinline', 'true'); video.setAttribute('muted', 'false');
        ...
        video.onplay = () => { console.log('Video empezó a reproducirse'); this.showNotification('Transmisión en vivo', 'success'); };
        video.onerror = (error) => { console.error('Error en video:', error); this.showNotification('Error al reproducir video', 'error'); };
    }
    setupDataConnection(conn) {
        conn.on('open', () => {
            ...
            conn.on('error', (err) => { console.error('Error en conexión de datos:', err); });
        });
    }
            ...
            switch(message.type) {
                ...
                case 'ping':
                    if (this.dataConnection) {
                        this.dataConnection.send(JSON.stringify({ type: 'pong', timestamp: message.timestamp, receivedAt: Date.now() }));
                    } break;
                case 'pong':
                    const latency = Date.now() - message.timestamp;
                    this.elements.latencyValue.textContent = `${latency} ms`; break;
            }
        } catch (error) { console.log('Mensaje recibido:', data); }
    }
    handleIncomingCall(call) {
        if (this.state.isEmitter && this.localStream) {
            call.answer(this.localStream); this.currentCall = call;
            console.log('✅ Aceptada llamada de:', call.peer);
            if (this.dataConnection) {
                this.dataConnection.send(JSON.stringify({ type: 'viewer-connected', viewerId: call.peer }));
            }
            this.state.connectedViewers.add(call.peer); this.updateConnectedClients();
            // Escuchar el audio del supervisor
            call.on('stream', (supervisorStream) => {
                ...
                if (!this.supervisorAudioElement) {
                    ...
                    this.supervisorAudioElement.autoplay = true; this.supervisorAudioElement.volume = 1.0;
                    this.supervisorAudioElement.style.display = 'none'; document.body.appendChild(this.supervisorAudioElement);
                }
                this.supervisorAudioElement.srcObject = supervisorStream;
                this.supervisorAudioElement.play().catch(e => { console.log('Audio del supervisor autoplay bloqueado:', e); });
                this.showNotification('Supervisor habilitó audio bidireccional', 'info');
            });
            call.on('close', () => {
                console.log('Llamada cerrada por supervisor:', call.peer);
                if (this.dataConnection) {
                    this.dataConnection.send(JSON.stringify({ type: 'viewer-disconnected', viewerId: call.peer }));
                }
                this.state.connectedViewers.delete(call.peer); this.updateConnectedClients();
                if (this.supervisorAudioElement) { this.supervisorAudioElement.remove(); this.supervisorAudioElement = null; }
            });
            call.on('error', (err) => {
            ...
            console.log('Emisor no está transmitiendo, rechazando llamada'); call.close();
        }
    }
    async startViewerAudio() {
        try {
            // 1. Obtener un nuevo stream de micrófono REAL y activado.
            const newAudioStream = await navigator.mediaDevices.getUserMedia({
                audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 2, sampleRate: 48000 },
                video: false
            });
            // 2. Detener el track de audio antiguo (el silencioso).
            if (this.localAudioStream) {
                this.localAudioStream.getTracks().forEach(track => track.stop());
            }
            // 3. Reemplazar el stream local con el nuevo.
    ...
    testAudio() {
        try {
            ...
            oscillator.frequency.value = 440; oscillator.type = 'sine'; gainNode.gain.value = 0.1;
            oscillator.start();
            setTimeout(() => { oscillator.stop(); this.showNotification('Prueba de audio completada', 'success'); }, 500);
        } catch (error) { console.error('Error en prueba de audio:', error); this.showNotification('Error en prueba de audio', 'error'); }
    }
    ...
    takeSnapshot() {
        const video = this.elements.remoteVideo;
        if (!video.srcObject || video.videoWidth === 0) { this.showNotification('No hay video para capturar', 'warning'); return; }
        const canvas = document.createElement('canvas');
        canvas.width = video.videoWidth; canvas.height = video.videoHeight;
        const ctx = canvas.getContext('2d'); ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
        const link = document.createElement('a');
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        link.download = `captura-${timestamp}.png`; link.href = canvas.toDataURL('image/png'); link.click();
    ...
    toggleMute() {
        if (this.elements.remoteVideo) {
            ...
            this.elements.btnMuteAudio.innerHTML = `<i class="fas fa-volume-${isMuted ? 'mute' : 'up'}"></i> ${isMuted ? 'Sonido: OFF' : 'Sonido: ON'}`;
    ...
    disconnectFromEmitter() {
        ...
        if (this.currentCall) { this.currentCall.close(); this.currentCall = null; }
        if (this.dataConnection) { this.dataConnection.close(); this.dataConnection = null; }
        if (this.remoteStream) { this.remoteStream.getTracks().forEach(track => track.stop()); this.remoteStream = null; this.elements.remoteVideo.srcObject = null; }
        if (this.localAudioStream) { this.localAudioStream.getTracks().forEach(track => track.stop()); this.localAudioStream = null; }
        this.state.isConnected = false; this.state.isAudioEnabled = false; this.state.hostPeerId = null;
        this.elements.connectionOverlay.classList.remove('hidden');
        this.elements.btnStartAudio.classList.add('hidden'); this.elements.btnStopAudio.classList.add('hidden');
        ...
        this.updateViewerStatus('Desconectado', 'disconnected'); this.updateConnectionState('Desconectado');
        this.stopUptimeTimer(); this.stopConnectionStatsMonitoring();
        this.showNotification('Desconectado de la transmisión', 'info');
    }
    handleMediaError(error) {
        let errorMsg = 'Error al acceder a los dispositivos';
        if (error.name === 'NotAllowedError') { errorMsg='Se denegó el acceso a la cámara/micrófono. Verifica los permisos del navegador.'; }
        else if (error.name === 'NotFoundError') { errorMsg='No se encontró cámara o micrófono.'; }
        else if (error.name === 'NotReadableError') { errorMsg='No se puede acceder al dispositivo. Puede estar en uso por otra aplicación.'; }
        else if (error.name === 'OverconstrainedError') { errorMsg='No se puede cumplir con las restricciones solicitadas.'; }
    ...
    handleConnectionError(error) {
        ...
        if (this.state.connectionAttempts < 3) {
            ...
            setTimeout(() => { if (this.state.hostPeerId) { this.attemptConnection(this.state.hostPeerId); } }, 2000 * this.state.connectionAttempts);
        } else {
    ...
    showErrorModal(message) { this.elements.errorMessage.textContent = message; this.elements.errorModal.classList.remove('hidden'); }
    hideErrorModal() { this.elements.errorModal.classList.add('hidden'); }
    retryConnection() { this.hideErrorModal(); this.state.connectionAttempts = 0; if (this.state.hostPeerId) { this.attemptConnection(this.state.hostPeerId); } }
    updateEmitterStatus(message, type = 'ready') { this.elements.emitterStatusBadge.textContent = message; this.elements.emitterStatusBadge.className = 'status-badge ' + type; }
    updateViewerStatus(message, type = 'disconnected') { this.elements.viewerStatusBadge.textContent = message; this.elements.viewerStatusBadge.className = 'status-badge ' + type; }
    updateConnectionStatus(status) { this.elements.connectionStatus.textContent = status; }
    updateConnectionState(state) { this.elements.connectionState.textContent = state; }
    updateQualityTag() {
        const quality = this.state.settings.videoQuality;
        const resolutions = { low: '480p', medium: '720p HD', high: '1080p FHD' };
        this.elements.qualityTag.textContent = resolutions[quality] || 'HD';
    }
    updateConnectedClients() {
        const container = this.elements.connectedClients; container.innerHTML = '';
        if (this.state.connectedViewers.size === 0) {
            container.innerHTML = `<div class="empty-state"><i class="fas fa-user-clock"></i><p>Esperando conexiones...</p></div>`; return;
        }
        this.state.connectedViewers.forEach(viewerId => {
            const client = document.createElement('div'); client.className = 'client-item';
            client.innerHTML = `<i class="fas fa-user-circle"></i><span>Supervisor ${viewerId.substring(0, 6)}</span><div class="client-status online"></div>`;
    ...
    updateConnectionStats() {
        ...
        if (this.dataConnection) {
            this.dataConnection.send(JSON.stringify({ type: 'ping', timestamp: Date.now() }));
        }
        if (this.state.isAudioEnabled) { this.elements.connectionStats.textContent = 'Audio bidireccional activo'; }
        else { this.elements.connectionStats.textContent = 'Audio unidireccional (solo emisor)'; }
    }
    startUptimeTimer() {
    ...
    stopUptimeTimer() { if (this.uptimeInterval) { clearInterval(this.uptimeInterval); this.uptimeInterval = null; } this.elements.uptime.textContent = '00:00'; }
    startConnectionStatsMonitoring() {
        this.statsInterval = setInterval(() => { if (this.state.isConnected) { this.updateConnectionStats(); } }, 3000);
    }
    stopConnectionStatsMonitoring() { if (this.statsInterval) { clearInterval(this.statsInterval); this.statsInterval = null; } }
    clearAllTimeouts() { this.connectionTimeouts.forEach(timeout => { clearTimeout(timeout); }); this.connectionTimeouts = []; }
    showNotification(message, type = 'info') {
        const notification = document.createElement('div'); notification.className = `notification ${type}`;
        notification.innerHTML = `<i class="fas fa-${this.getNotificationIcon(type)}"></i><div class="notification-content">${message}</div><button class="notification-close">&times;</button>`;
    ...
    handleDisconnection() {
        this.state.isConnected = false; this.elements.connectionOverlay.classList.remove('hidden');
        this.updateViewerStatus('Desconectado', 'disconnected'); this.updateConnectionState('Desconectado');
        this.stopUptimeTimer(); this.stopConnectionStatsMonitoring();
        if (this.state.settings.autoReconnect && this.state.hostPeerId) {
            ...
            setTimeout(() => { if (this.state.hostPeerId && !this.state.isConnected) { this.attemptConnection(this.state.hostPeerId); } }, 3000);
        }
    }
}
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (connectTo) {
        setTimeout(() => {
            ...
            setTimeout(() => { window.app.connectToEmitter(); }, 1000);
        }, 500);
    }
});
